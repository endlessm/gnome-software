...
typedef gboolean	 (*GsPluginCopyFunc)		(GsPlugin	*plugin,
							 ...
							 GCancellable	*cancellable,
							 GError		**error);
typedef gboolean	 (*GsPluginOsGetCopyableFunc)	(GsPlugin	*plugin,
							 GFile		*copy_dest,
							 gboolean	*result,
							 GCancellable	*cancellable,
							 GError		**error);
			     ...
			     GError **error)
{
	...
	case GS_PLUGIN_ACTION_COPY:
		...
		break;
	case GS_PLUGIN_ACTION_OS_GET_COPYABLE:
		{
			GsPluginOsGetCopyableFunc plugin_func = func;
			ret = plugin_func (plugin,
					   gs_plugin_job_get_copy_dest (helper->plugin_job),
					   &helper->boolean_retval,
					   cancellable, &error_local);
				    ...
				    GError **error)
{
	...
	for (guint i = 0; i < priv->plugins->len; i++) {
		...
		if (gs_plugin_get_symbol (plugin, "gs_plugin_refine_app") != NULL ||
		    gs_plugin_get_symbol (plugin, "gs_plugin_refine_wildcard") != NULL) {
...
static gboolean
gs_plugin_loader_os_get_copyable_run (GsPluginLoaderHelper *helper,
				      GCancellable *cancellable,
				      GError **error)
{
	GsPluginLoaderPrivate *priv = gs_plugin_loader_get_instance_private (helper->plugin_loader);
	...
	for (guint i = 0; i < priv->plugins->len; i++) {
		...
		helper->function_name = "gs_plugin_os_get_copyable";
		if (!gs_plugin_loader_call_vfunc (helper, plugin, NULL, NULL,
						  GS_PLUGIN_REFINE_FLAGS_DEFAULT,
						  cancellable, error)) {
			return FALSE;
...
static void
gs_plugin_loader_job_os_get_copyable_thread_cb (GTask *task,
						gpointer object,
						gpointer task_data,
						GCancellable *cancellable)
{
	...
	if (!gs_plugin_loader_os_get_copyable_run (helper, cancellable, &error)) {
		g_task_return_error (task, error);
		return;
	}
 ...
 *
 * This method calls gs_plugin_os_get_copyable() for the @plugin belonging to
 * @plugin_job if the plugin implements it.
 **/
void
gs_plugin_loader_job_os_get_copyable_async (GsPluginLoader *plugin_loader,
					    GsPluginJob *plugin_job,
					    GCancellable *cancellable,
					    GAsyncReadyCallback callback,
					    gpointer user_data)
{
	...
	task = g_task_new (plugin_loader, cancellable, callback, user_data);
	g_task_set_task_data (task, helper, (GDestroyNotify) gs_plugin_loader_helper_free);
	g_task_run_in_thread (task, gs_plugin_loader_job_os_get_copyable_thread_cb);
}

/**
 * gs_plugin_loader_job_os_get_copyable_finish:
 *
 * Return value: the value of the plugin's gs_plugin_os_get_copyable(), if it
 * exists, or %FALSE if not.
 **/
gboolean
gs_plugin_loader_job_os_get_copyable_finish (GsPluginLoader *plugin_loader,
					      GAsyncResult *res,
					      GError **error)
{
	g_return_val_if_fail (GS_IS_PLUGIN_LOADER (plugin_loader), FALSE);
