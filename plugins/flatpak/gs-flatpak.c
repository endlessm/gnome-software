...
static gboolean update_app (GsFlatpak *self,
			    GsApp *app,
...
static FlatpakRef *
get_app_ref_in_installation (FlatpakInstallation *installation,
			     GsApp *app,
			     GCancellable *cancellable,
			     GError **error)
{
	g_autoptr(GPtrArray) xremotes = NULL;
	FlatpakRefKind kind = gs_flatpak_app_get_ref_kind (app);
	const gchar *name = gs_flatpak_app_get_ref_name (app);
	const gchar *arch = gs_flatpak_app_get_ref_arch (app);
	const gchar *branch = gs_flatpak_app_get_ref_branch (app);

	g_return_val_if_fail (name != NULL, NULL);
	g_return_val_if_fail (branch != NULL, NULL);

	/* look at each remote xref */
	xremotes = flatpak_installation_list_remotes (installation, cancellable, error);
	if (xremotes == NULL)
		return NULL;
	...
	for (guint i = 0; i < xremotes->len; ++i) {
		...
		for (guint j = 0; j < refs_remote->len; ++j) {
			...
			if (flatpak_ref_get_kind (FLATPAK_REF (xref)) == kind &&
				...
				return g_object_ref (xref);
			}
		}
	}

	g_set_error (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND,
		     "%s has no ref in %s installation",
		     gs_app_get_unique_id (app),
		     (flatpak_installation_get_is_user (installation) ? "user" : "system"));
	return NULL;
}
...
static void
gs_plugin_refine_item_scope (GsFlatpak *self,
			     GsApp *app)
{
	AsAppScope scope;
	g_autoptr(FlatpakRef) ref = NULL;
	g_autoptr(FlatpakInstallation) system_installation = NULL;
	g_autoptr(FlatpakInstallation) user_installation = NULL;
	...
	/* try to find where the runtime is installed or available */

	if (self->scope == AS_APP_SCOPE_USER) {
		system_installation = gs_flatpak_get_installation_counterpart (self, NULL, &local_error);
		user_installation = g_object_ref (self->installation);
	} else {
		system_installation = g_object_ref (self->installation);
		user_installation = gs_flatpak_get_installation_counterpart (self, NULL, &local_error);
	}

	if (system_installation == NULL || user_installation == NULL) {
		g_warning ("Failed to get installation: %s", local_error->message);
		return;
	}

	/* look for installed refs first */
	ref = FLATPAK_REF (get_installed_ref_for_app (system_installation, app, NULL, NULL));
	if (ref != NULL) {
		scope = AS_APP_SCOPE_SYSTEM;
		goto out;
	}

	ref = FLATPAK_REF (get_installed_ref_for_app (user_installation, app, NULL, NULL));
	if (ref != NULL) {
		scope = AS_APP_SCOPE_USER;
		goto out;
	}

	ref = get_app_ref_in_installation (system_installation, app, NULL, NULL);
	if (ref != NULL) {
		scope = AS_APP_SCOPE_SYSTEM;
		goto out;
	}

	ref = get_app_ref_in_installation (user_installation, app, NULL, NULL);
	if (ref != NULL) {
		scope = AS_APP_SCOPE_USER;
		goto out;
	}
...
static gboolean
get_server_updates (GsFlatpak *self,
		    GsAppList *list,
		    GCancellable *cancellable,
		    GError **error)
{
...
static gboolean
get_downloaded_updates (GsFlatpak *self,
			GsAppList *list,
			GCancellable *cancellable,
			GError **error)
{
...
static gboolean
get_all_updates (GsFlatpak *self,
		 GsAppList *list,
		 GCancellable *cancellable,
		 GError **error)
{
	gboolean ret = get_downloaded_updates (self, list, cancellable, error);
	if (!ret)
		return FALSE;
...
gs_flatpak_add_updates (GsFlatpak *self, GsAppList *list,
			...
			GError **error)
{
	return get_downloaded_updates (self, list, cancellable, error);
}
		    ...
		    GError **error)
{
	...
	if (!get_all_updates (self, list, cancellable, error))
		return FALSE;

	num_updates = gs_app_list_length (list);
	if (num_updates == 0) {
		g_debug ("No updates to be done on refresh");
		return TRUE;
	}

	g_debug ("Got %u apps with updates on refresh; updating them now",
		 num_updates);
	for (guint i = 0; i < num_updates; ++i) {
		GsApp *app = NULL;
		g_autoptr(GError) local_error = NULL;

		if (g_cancellable_is_cancelled (cancellable)) {
			g_debug ("Cancelling %u remaining updates",
				 num_updates - i);
			break;
		}

		app = gs_app_list_index (list, i);
		if (!update_app (self, app, cancellable, &local_error))
			g_debug ("Failed to update %s automatically: %s",
				 gs_app_get_unique_id (app),
				 local_error->message);
	}
...
gs_flatpak_get_installed_ref (GsFlatpak *self,
			      ...
			      GError **error)
{
	return get_installed_ref_for_app (self->installation, app, cancellable, error);
}
					   ...
					   GError **error)
{
	...
	/* if the app is installed but missing its runtime, then it's been set to show
	 * as available in order for it to be repaired (for the missing runtime to be
	 * installed) */
	is_repair = !is_update && g_hash_table_contains (hash_installed, app_ref_display);
	if (is_repair) {
	...
	if (services_app != NULL) {
		...
		if (is_repair) {
			services_runtime = gs_app_get_runtime (services_app);
		} else {
			if (!gs_flatpak_refine_runtime_for_install (self, services_app,
								 cancellable, error))
				return FALSE;
...
gs_flatpak_update_app (GsFlatpak *self,
		       ...
		       GError **error)
{
	return update_app (self, app, cancellable, error);
}
