...
_app_state_notify_cb (GsApp *app, GParamSpec *pspec, gpointer user_data)
{
	...
	GsUpdatesPage *page = GS_UPDATES_PAGE (g_object_get_data (G_OBJECT (app_row),
								  "page"));
...
_add_app_row (GsUpdatesPage *self, GsApp *app)
{
	...
	g_object_set_data (G_OBJECT (app_row), "page", self);
                                   ...
                                   GsUpdatesPage *self)
{
	...
	GCancellable *cancellable = gs_app_get_cancellable (app);

	/* we need to reassign the cancellable to cover the case where the
	 * cancel button shows up because the upgrade GsApp changed to the
	 * "installing" state without the user having clicked "download" (for
	 * example in automatic downloads) */
	g_set_object (&self->cancellable_upgrade_download, cancellable);
