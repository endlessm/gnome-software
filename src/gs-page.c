		       ...
		       gpointer user_data)
{
	...
	if (GS_PAGE_GET_CLASS (page)->app_copied != NULL)
		GS_PAGE_GET_CLASS (page)->app_copied (page, helper->app);
}
		      ...
		      gpointer user_data)
{
	...
	if (GS_PAGE_GET_CLASS (page)->os_copied != NULL)
		GS_PAGE_GET_CLASS (page)->os_copied (page);
}
...
gs_page_remove_app (GsPage *page, GsApp *app, GCancellable *cancellable)
{
	...
	gtk_dialog_add_button (GTK_DIALOG (dialog), _("Uninstall"), GTK_RESPONSE_OK);
