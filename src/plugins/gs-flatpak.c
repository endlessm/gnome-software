...
get_file_checksum (const char *path)
{
	...
	return g_compute_checksum_for_string (G_CHECKSUM_SHA256, contents,
					      length);
}
			      ...
			      GCancellable *cancellable, GError **error)
{
	...
	for (i = 0; i < xremotes->len; i++) {
		...
		if (!appstream_checksum_2 ||
		    g_strcmp0 (appstream_checksum_1, appstream_checksum_2) != 0) {
			if (!changed_remotes) {
				changed_remotes =
					g_ptr_array_new_full (xremotes->len,
							      NULL);
			}

			g_ptr_array_add (changed_remotes,
					 (char *) remote_name);
...
gs_flatpak_app_install (GsFlatpak *self,
			...
			GError **error)
{
	if (!gs_flatpak_app_install_with_progress (self, app,
...
gs_flatpak_update_app (GsFlatpak *self,
		       ...
		       GError **error)
{
	if (!gs_flatpak_update_app_with_progress (self, app,
