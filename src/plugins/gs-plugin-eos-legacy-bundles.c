...
is_app_id (const char *appid)
{
	...
	static const char *reserveddirs[] = { "bin", "games", "share",
					      "lost+found", "xdg", };
...
static void
gs_plugin_set_categories_from_desktop_app_info (GsApp *app,
...
gs_plugin_set_icon_from_app_info (GsApp *app, GAppInfo *app_info)
{
	...
	if (!icon) {
		g_warning ("Could not get icon from app info for app '%s'",
			   gs_app_get_name (app));
		return;
	}

	info = gtk_icon_theme_lookup_by_gicon (theme, icon, 64,
					       GTK_ICON_LOOKUP_GENERIC_FALLBACK);

	if (!info) {
		g_warning ("Could not get icon info from app info for app '%s'",
			   gs_app_get_name (app));
		goto cleanup;
	}
...
gs_plugin_set_app_info_from_bundle_info_file (GsApp *app, const gchar *info_file)
{
	...
	if (!g_key_file_load_from_file (key_file, info_file, G_KEY_FILE_NONE, &error)) {
		g_warning ("Error loading info file '%s': %s", info_file,
			   error->message);
		g_error_free (error);
		goto cleanup;
	}

	value = g_key_file_get_value (key_file, EOS_BUNDLE_KEY_FILE_GROUP,
				      "version", NULL);
	if (value) {
                         ...
                         GError **error)
{
	...
	while ((appid = g_dir_read_name (dir)) != NULL) {
		...
		if (!is_app_id (appid)) {
			g_debug ("Skipping '%s/%s': not a valid app directory",
				 prefix, appid);
			continue;
		}
                      ...
                      GError **error)
{
	...
	if (!remove_app_from_shell (plugin, app, cancellable, error)) {
		if (error)
			g_debug ("Failed to remove '%s' from shell: %s",
